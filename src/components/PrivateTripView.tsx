  ...
  return (
    <div>
      ...
      >
        ...
        <span className="font-semibold">Drop photos & videos here, or click to add</span>
        <span className="font-hand text-lg">everything stays inside the circle until the editor says otherwise</span>
      </button>
            ...
            <figure key={m.id} className="group">
              <div className={`photo-card relative rounded-sm ${i % 2 ? "rotate-1" : "-rotate-1"}`}>
                ...
                <div className="relative aspect-[4/3] overflow-hidden rounded-[3px]">
                  ...
                  <span className="absolute left-2.5 top-2.5">
                    {m.isPublic ? (
                      ...
                      <Badge tone="faint"><IconLock className="h-3 w-3" /> private</Badge>
                    )}
                  </span>
                    ...
                    {!m.isPublic && !pend && (
                      <>
                        <ActionBtn
                          ...
                          onClick={() =>
                            pitch(m, "make_public", "Pitched to the editor — it goes public only if approved.")
                          }
                        >
                    ...
                    {mine && m.isPublic && !pend && (
                      <>
                        <ActionBtn
                          ...
                          onClick={() =>
                            pitch(m, "retract_public", "Retraction requested — the editor decides what leaves the page.")
                          }
                        >
                        ...
                        <ActionBtn
                          ...
                          onClick={() =>
                            pitch(m, "delete_public", "Deletion of public work needs the editor's sign-off. Request sent.")
                          }
                        >
                          <IconTrash className="h-4 w-4" />
                        </ActionBtn>
                ...
                <figcaption className="flex items-baseline justify-between gap-3 px-1 pt-2.5">
                  <span className="font-hand text-lg leading-snug text-ink-soft">{m.caption}</span>
              ...
              {pend && (
                <p className="mt-2.5 px-1">
                  <Badge tone="gold">
                    ...
                    {pend === "make_public" && "pitch sent — awaiting the editor"}
                    {pend === "retract_public" && "retraction awaiting the editor"}
